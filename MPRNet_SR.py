...
#initialized with pre-trained model of our method (CSIT)
# Only 2 changes were performed on MPRNet:
# 1) added SR part of EDPN
# 2) adjusted the architecture of ORSNET to the MPRNet provided in NTIRE 2021
# 3) added 120 RCAB_MSCAM blocks after ORSNET, added residual connections + conv in ResidualGroup_RCAB_MSCAM
#
...
class MPRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=96, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3, reduction=4, bias=False):
        ...
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True) # as in EDPN
        self.SRNet_HRconv = nn.Conv2d(64, 64, 3, 1, 1, bias=False) # as in EDPN
